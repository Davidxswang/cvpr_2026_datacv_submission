...
If a run is interrupted by provider-side failures (e.g. frequent error_code=429) and you want to repair only the failed samples in the same experiment directory with a different model, rerun with `--allow-mixed-model-repair` alongside the new model name.
